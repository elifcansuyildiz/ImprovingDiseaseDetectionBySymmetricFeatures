...
class SiameseNetwork(pl.LightningModule):
    ...
    def forward(self, img, img_flipped):
        ...
        probability_map = self.feature_fusion(img_encodings, img_f_encodings)
        if self.include_feature_comparison:
            distance_map = self.feature_comparison(img_encodings, img_f_encodings)
...
class Classifier(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        probability_map, distance_map = self.model(img, flipped_img)
        
        with torch.no_grad():
            target_probmap = F.interpolate(target_map, size=(probability_map.shape[-1], probability_map.shape[-1]), mode="area")
            target_distmap = F.interpolate(target_map, size=(distance_map.shape[-1], distance_map.shape[-1]), mode="area")
...
if __name__ == "__main__":
    ...
    contrastive_loss_function = ContrastiveLoss(margin=5.0)
